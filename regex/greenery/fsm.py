    ...
    def star(self):
        ...
        def follow(state, symbol):
            ...
            for substate in state:
                ...
                if substate in self.finals and self.initial in self.map and symbol in self.map[self.initial]:
    ...
    def times(self, multiplier):
        ...
        def follow(current, symbol):
            ...
            for (substate, iteration) in current:
                if iteration < multiplier and substate in self.map and symbol in self.map[substate]:
...
def parallel(fsms, test):
    ...
    # Jiarong: deduplicate anything_else
    has_anything_else = False
    copied = []
    for tmp in alphabet:
        if isinstance(tmp, anything_else_cls):
            if not has_anything_else:
                copied.append(anything_else)
    ...
    def follow(current, symbol):
        ...
        for i in range(len(fsms)):
            ...
            if i in current and current[i] in fsms[i].map and actual_symbol in fsms[i].map[current[i]]:
